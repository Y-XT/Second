...
def transformation_from_gtmsrtpose(T_prior, scale, t_res, invert=False):
    """Fuse external pose with predicted residual translation and scale.

    T_prior: [B, 4, 4] external pose (rotation preserved)
    scale:   [B, 1, 1] or [B, 1] scalar for prior translation
    t_res:   [B, 1, 3] or [B, 3] residual translation
    """
    t = t_res.clone()
    if invert:
        t *= -1
