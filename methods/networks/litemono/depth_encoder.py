...
class LiteMono(nn.Module):
    ...
    """
    @staticmethod
    def _select_dilation_schedule(model, height, width):
        schedules = {
            'lite-mono': {
                (192, 640): [[1, 2, 3], [1, 2, 3], [1, 2, 3, 1, 2, 3, 2, 4, 6]],
                (320, 1024): [[1, 2, 5], [1, 2, 5], [1, 2, 5, 1, 2, 5, 2, 4, 10]],
            },
            'lite-mono-small': {
                (192, 640): [[1, 2, 3], [1, 2, 3], [1, 2, 3, 2, 4, 6]],
                (320, 1024): [[1, 2, 5], [1, 2, 5], [1, 2, 5, 2, 4, 10]],
            },
            'lite-mono-tiny': {
                (192, 640): [[1, 2, 3], [1, 2, 3], [1, 2, 3, 2, 4, 6]],
                (320, 1024): [[1, 2, 5], [1, 2, 5], [1, 2, 5, 2, 4, 10]],
            ...
            'lite-mono-8m': {
                (192, 640): [[1, 2, 3], [1, 2, 3], [1, 2, 3, 1, 2, 3, 2, 4, 6]],
                (320, 1024): [[1, 2, 3], [1, 2, 3], [1, 2, 3, 1, 2, 3, 2, 4, 6]],
            },
        }

        if model not in schedules:
            raise ValueError("Unsupported LiteMono variant: {}".format(model))

        variant_schedules = schedules[model]
        if (height, width) in variant_schedules:
            return variant_schedules[(height, width)]

        target_area = height * width
        nearest_size = min(
            variant_schedules,
            key=lambda size: abs(size[0] * size[1] - target_area)
        )
        print("[LiteMono] unsupported input size {}x{} for {}, using dilation schedule from {}x{}".format(
            height, width, model, nearest_size[0], nearest_size[1]
        ))
        return variant_schedules[nearest_size]
    ...
    def __init__(self, in_chans=3, model='lite-mono', height=192, width=640,
        ...
        self.dilation = self._select_dilation_schedule(model, height, width)
