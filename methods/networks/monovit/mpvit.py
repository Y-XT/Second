...
def _count_matched_tensors(module, checkpoint_state_dict):
    ...
    for k, v in model_state.items():
        ...
        if vv is not None and hasattr(vv, "shape") and v.shape == vv.shape:
            matched += 1
...
def mpvit_xsmall(**kwargs):
    ...
    checkpoint = torch.load('./ckpt/mpvit_xsmall.pth', map_location=lambda storage, loc: storage)
    checkpoint_state = _extract_checkpoint_state_dict(checkpoint)
    matched_num, total_num = _count_matched_tensors(model, checkpoint_state)
    logger = get_root_logger()
    load_state_dict(model, checkpoint_state, strict=False, logger=logger)
    model.pretrained_num_loaded = matched_num
    model.pretrained_num_total = total_num
    print(
        "[init_models][pretrained] encoder=MPViT(xsmall): loaded "
        f"(ckpt=./ckpt/mpvit_xsmall.pth, matched={matched_num}/{total_num})"
    )
    del checkpoint
    del logger
    model.default_cfg = _cfg_mpvit()
...
def mpvit_small(**kwargs):
    ...
    checkpoint = torch.load('/mnt/data_nvme3n1p1/PycharmProjects/monodepth2/ckpt/mpvit_small.pth', map_location=lambda storage, loc: storage)
    checkpoint_state = _extract_checkpoint_state_dict(checkpoint)
    matched_num, total_num = _count_matched_tensors(model, checkpoint_state)
    logger = get_root_logger()
    load_state_dict(model, checkpoint_state, strict=False, logger=logger)
    ...
    model.pretrained_num_total = total_num
    print(
        "[init_models][pretrained] encoder=MPViT(small): loaded "
        f"(ckpt=/mnt/data_nvme3n1p1/PycharmProjects/monodepth2/ckpt/mpvit_small.pth, "
        f"matched={matched_num}/{total_num})"
    )
    del checkpoint
    del logger
    model.default_cfg = _cfg_mpvit()
