...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--exp_dir', type=str, 
                        #default='/mnt/data_nvme3n1p1/mono_weights/weights/UAVid_Germany/mrfedepth_uavid_tridataset_512x288_bs8_lr1e-04_e40_step20_ru9k',
                        default='/mnt/data_nvme3n1p1/mono_weights/weights/UAVid_China/mrfedepth_uavid_tridataset_512x288_bs8_lr1e-04_e40_step20_zy49',
    ...
    parser.add_argument('--model_name', type=str, default='auto',
                        help='模型分支（auto | GasMono* | Madhuanand* | MRFEDepth* | MonoViT* | LiteMono* | SPIDepth* | MonoDepth2*）')
...
def _infer_model_branch_from_encoder_sd(enc_sd: Dict[str, object]) -> Optional[str]:
    ...
    # Monodepth2 / ResNet encoder 常见 key
...
def load_model(args: argparse.Namespace, device: torch.device):
    ...
    if os.path.isfile(encoder_path):
        ...
        # MRFEDepth checkpoints use DepthEncoder/DepthDecoder naming.
    ...
    if model_branch == 'MRFEDepth':
        ...
        print('Loading MRFEDepth encoder:', depth_encoder_path)
        ...
        print('Loading GasMono encoder:', encoder_path)
        print('Loading GasMono decoder:', decoder_path)

        encoder = networks.mpvit_small()
        encoder.num_ch_enc = [64, 128, 216, 288, 288]
        enc_sd = encoder_sd if encoder_sd is not None else _load_state_dict_file(encoder_path, device)
        _load_partial_state_dict(encoder, enc_sd, 'GasMono.encoder')
        encoder.to(device).eval()
        feed_height = int(enc_sd.get('height', DEFAULT_FEED_HEIGHT))
        feed_width = int(enc_sd.get('width', DEFAULT_FEED_WIDTH))
        ...
        if depth_sd_probe is not None and os.path.abspath(decoder_path) == os.path.abspath(depth_path):
            dec_sd = depth_sd_probe
        else:
            dec_sd = _load_state_dict_file(decoder_path, device)
        _load_partial_state_dict(decoder, dec_sd, 'GasMono.depth')
        decoder.to(device).eval()
        ...
        feed_height = int(enc_sd.get('height', DEFAULT_FEED_HEIGHT))
        feed_width = int(enc_sd.get('width', DEFAULT_FEED_WIDTH))
        # 评测统一固定使用 LiteMono-8m（与对比实验保持一致，不做自动变体判断）。
        encoder = networks.LiteMono(model='lite-mono-8m', width=feed_width, height=feed_height)
        _load_partial_state_dict(encoder, enc_sd, 'LiteMono.encoder(lite-mono-8m)')
        encoder.to(device).eval()

        decoder = networks.DepthDecoder_litemono(np.array(encoder.num_ch_enc), [0, 1, 2])
        dec_sd = _load_state_dict_file(decoder_path, device)
        _load_partial_state_dict(decoder, dec_sd, 'LiteMono.decoder(lite-mono-8m)')
        decoder.to(device).eval()

    elif model_branch == 'SPIDepth':
        decoder_path = depth_path
        enc_sd = encoder_sd if encoder_sd is not None else _load_state_dict_file(encoder_path, device)
        is_lite, num_layers, num_features, model_dim = _infer_spidepth_encoder_cfg(enc_sd)
        if is_lite:
        ...
        _load_partial_state_dict(encoder, enc_sd, 'SPIDepth.encoder')
        encoder.to(device).eval()
        feed_height = int(enc_sd.get('height', DEFAULT_FEED_HEIGHT))
        feed_width = int(enc_sd.get('width', DEFAULT_FEED_WIDTH))

        dec_sd = _load_state_dict_file(decoder_path, device)
        in_channels, embedding_dim, patch_size, query_nums, dim_out, transformer_ff_dim = _infer_spidepth_decoder_cfg(dec_sd)
        ...
        _load_partial_state_dict(decoder, dec_sd, 'SPIDepth.depth')
        decoder.to(device).eval()
        ...
        enc_sd = encoder_sd if encoder_sd is not None else _load_state_dict_file(encoder_path, device)
        _load_partial_state_dict(encoder, enc_sd, 'Madhuanand.encoder')
        encoder.to(device).eval()
        feed_height, feed_width = enc_sd['height'], enc_sd['width']

        decoder = networks.DepthDecoder_3d(num_ch_enc=encoder.num_ch_enc, scales=range(4))
        dec_sd = depth_sd_probe if depth_sd_probe is not None else _load_state_dict_file(decoder_path, device)
        _load_partial_state_dict(decoder, dec_sd, 'Madhuanand.decoder')
        decoder.to(device).eval()
        ...
        encoder = networks.ResnetEncoder(18, False)
        enc_sd = encoder_sd if encoder_sd is not None else _load_state_dict_file(encoder_path, device)
        _load_partial_state_dict(encoder, enc_sd, 'MonoDepth2.encoder')
        encoder.to(device).eval()
        feed_height, feed_width = enc_sd['height'], enc_sd['width']
...
def _seq_name_from_img_path(img_path: str, root: str) -> str:
    rel = os.path.relpath(img_path, root)
    return rel.split(os.sep)[0]


def _build_next_frame_map_from_pairs(pairs: List[Tuple[str, str, str, Optional[str]]],
                                     root: str) -> Dict[str, str]:
    """为每张图构建同序列下一帧路径；末帧回退为自身。"""
    seq_to_paths: Dict[str, List[str]] = {}
    for img_path, _, _, _ in pairs:
        seq = _seq_name_from_img_path(img_path, root)
        seq_to_paths.setdefault(seq, []).append(img_path)

    next_map: Dict[str, str] = {}
    for paths in seq_to_paths.values():
        ordered = sorted(dict.fromkeys(paths))
        if not ordered:
            continue
        for idx, path in enumerate(ordered):
            next_map[path] = ordered[idx + 1] if idx + 1 < len(ordered) else path
    return next_map
...
def _infer_norm_cfg(opt: argparse.Namespace) -> Dict[str, object]:
    """基于关键词自动推断是否需要归一化。"""
    names = []
    for key in ['encoder', 'backbone', 'model', 'model_name', 'teacher',
    ...
    if 'dino' in joined:
        ...
        return {'normalize': True, 'norm_mode': 'imagenet'}
...
def _log_norm_decision(opt: argparse.Namespace, cfg: Dict[str, object]) -> None:
    ...
    if cfg.get('norm_mode') == 'imagenet':
        print('[Normalize] ON  | mode=ImageNet | reason=自动启用（检测到 *dino*）')
    else:
        ...
        print(f'[Normalize] ON  | mode={mode} | mean={mean} | std={std} | reason=自动启用（检测到 *dino*）')
...
def test_simple(args: argparse.Namespace) -> None:
    ...
    if model_branch == "Madhuanand":
        source_path_map = _build_next_frame_map_from_pairs(pairs, args.gt_folder)
        fallback_self = sum(1 for img_path, _, _, _ in pairs if source_path_map.get(img_path, img_path) == img_path)
        print(f"[Madhuanand] using paired source frame for inference (self-fallback: {fallback_self}/{len(pairs)})")
