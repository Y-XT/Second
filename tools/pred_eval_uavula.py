...
def load_model(args: argparse.Namespace, device: torch.device):
    ...
    if model_branch == "MonoViT":
        ...
        _load_partial_state_dict(encoder, encoder_sd, "MonoViT.encoder")

        encoder.to(device).eval()
        feed_height = int(encoder_sd.get('height', DEFAULT_FEED_HEIGHT))
        feed_width  = int(encoder_sd.get('width', DEFAULT_FEED_WIDTH))
